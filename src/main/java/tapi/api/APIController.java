...
public class APIController
{
    ...
    {
        Credentials credentials = Credentials.create(""); //Must be different from key used to deploy
        String keyAddress = credentials.getAddress();

        byte[] buffer = new byte[512];
        //establish connection to server
        //try (Socket socket = new Socket("http://scriptproxy.smarttokenlabs.com", 8003))
        try (Socket socket = new Socket("192.168.50.9", 8003))
        {
